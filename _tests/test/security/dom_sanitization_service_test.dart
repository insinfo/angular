...
void main() {
  ...
  group('DOM sanitizer with safe proto support', () {
    ...
    group('sanitizeHtml', () {
      ...
      test('passes SafeHtml through untouched', () {
        expect(
            sanitizer
                .sanitizeHtml(sanitizer.bypassSecurityTrustHtml(unsafeHtml)),
            unsafeHtml);
      });
    ...
    group('sanitizeUrl', () {
      ...
      test('passes SafeUrl through untouched', () {
        expect(
            sanitizer.sanitizeUrl(sanitizer.bypassSecurityTrustUrl(unsafeUrl)),
            unsafeUrl);
      });
    ...
    group('bypassSecurityTrustHtml', () {
      test('returns object with expected toString', () {
        const html = '<div alt="a"> <script>alert()</script> </div> <br>';
        expect(sanitizer.bypassSecurityTrustHtml(html).toString(), html);
      });
    });

    group('bypassSecurityTrustStyle', () {
      test('returns object with expected toString', () {
        const style = 'background-image: url("//dog.png")';
        expect(sanitizer.bypassSecurityTrustStyle(style).toString(), style);
      });
    });

    group('bypassSecurityTrustUrl', () {
      test('returns object with expected toString', () {
        const url = 'http://www.google.com/something';
        expect(sanitizer.bypassSecurityTrustUrl(url).toString(), url);
      });
    });

    group('bypassSecurityTrustResourceUrl', () {
      test('returns object with expected toString', () {
        const url = 'http://www.google.com/something.js';
        expect(sanitizer.bypassSecurityTrustResourceUrl(url).toString(), url);
      });
    });
  });
}
